    ...
    <div className="max-w-3xl mx-auto p-6 pt-24">
      <Card className="w-full">
        ...
        <CardContent>
          {questions.map((question, idx) => (
            <div key={idx} className="my-4 p-4 border rounded-md">
              <Label htmlFor={`question-${idx}`} className="mb-2">Question{idx + 1}</Label>
              <Input
                id={`question-${idx}`}
              ...
              <Select
                value={question.type}
                onValueChange={(value) => handleTypeChange(idx, value as Question["type"])}
              >
                <SelectTrigger>
              ...
              {question.type === "MULTIPLE" && (
                <div className="mt-4">
                  ...
                  {question.options.map((option, optionIdx) => (
                    <div key={optionIdx} className="flex items-center space-x-2 mt-2">
                      <Input
                        placeholder={`Option ${optionIdx + 1}`}
                        value={option}
                        onChange={(e) => handleOptionChange(idx, optionIdx, e.target.value)}
                      />
                      <Button variant="outline" onClick={() => removeOption(idx, optionIdx)}>
                        <CircleX />
                      </Button>
                    </div>
                  ))}
                  <Button variant="outline" className="mt-2" onClick={() => addOption(idx)}>
                    <Plus /> Add Option
                  </Button>
                </div>
              )}
              ...
              {question.type === "RATING" && (
                <p className="text-sm text-gray-600 mt-2">This question will have a 1-5 rating scale.</p>
              )}
            </div>
          ))}
        ...
        <CardFooter className="flex justify-between">
          <Button variant="outline" onClick={() => router.push(`/room/create/${roomId}`)}>
            Skip
          </Button>
